...
{{ toYaml .excludePaths | indent 10  }}
    ...
    patches:
      ...
      # The volumes above are provisioned root-owned, so a non-root dev
      # container cannot sync into them. fsGroup makes kubelet fix group
      # ownership of every mounted volume on each pod start, which keeps the
      # dev container non-root and needs no privileged init container.
      - op: add
